/*
 *
 *  This lab might not work properly without fiddling with things, since I just copied it from a different project.
 *
 */
...
static void receive_task(void* pvParameters) {
	...
	while (true) {
		uint32_t const len = USB_receive(str, 79);
		str[len] = 0;
		ITM_write(str);
...
int main(void) {
	...
	LpcUart* uart = new LpcUart({ LPC_USART0, 115200, UART_CFG_DATALEN_8 | UART_CFG_PARITY_NONE | UART_CFG_STOPLEN_1, false,
		{ 0, 18 }, { 0, 13 } });
